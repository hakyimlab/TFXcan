...
TF <- 'FOXA1'
id <- 'kawakami'
kawakami_data_dir <- glue('{project_dir}/data/train-test-val/kawakami/data_{data_date}')
```
...
```{r}
ground_truth_path <- glue('{project_dir}/../impact_pipeline/motif_intervals/kawakami/intervals_{data_date}/ground_truth/{id}_{TF}_*.txt') |> Sys.glob()
ground_truth_path
```
...
Split into 80-20 (for now, no splitting)
```{r}
set.seed(2022)
...
```
```{r}
data.table::fwrite(x=train, file=glue('{save_dir}/train_enet_{Sys.Date()}.csv'), quote=F, row.names=F)
#gzip(glue('{save_dir}/train_enet_{Sys.Date()}.csv'), destname=glue('{save_dir}/train_enet_{Sys.Date()}.csv.gz'))
cmd <- glue("pigz -k {save_dir}/train_enet_{Sys.Date()}.csv")
system(cmd)
...
```{r}
dt_train <- temp_dt[1:1000, ] #data.table::fread(data_file)
print(dim(dt_train))

# split the data
X_train <- dt_train[, -c(1,2,3)] |> as.matrix()
y_train <- dt_train[, c(1,2,3)] |> as.data.frame()
print(head(y_train)) ; y_train$class |> table()
```

```{r}
set.seed(2022)
cv_model <- glmnet::cv.glmnet(x=X_train, y=y_train$class, family = "binomial", type.measure = "auc", alpha = 0.5, keep=T, parallel=F, nfolds=3, trace.it=F)
print(cv_model)
```
...
```{r}
linear_cvfit <- cv.glmnet(x=X_train, y=nbc, type.measure = "mse", nfolds = 3, alpha=0.5)

print(linear_cvfit) ; plot(linear_cvfit)
```

```{r}
linear_glmfit <- glmnet(x=X_train, y=nbc, nlambda=50, alpha=1)
print(linear_glmfit)
```

```{r}
dt_test <- temp_dt[1001:2000, ] #data.table::fread(data_file)
print(dim(dt_test))

# split the data
X_test <- dt_test[, -c(1,2,3)] |> as.matrix()
y_test <- dt_test[, c(1,2,3)] |> as.data.frame()
print(head(y_test)) ; y_test$class |> table()
```

```{r}
train_predictions <- predict(linear_cvfit, X_train, type='response', s=linear_cvfit$lambda.min)
```

```{r}
test_predictions <- predict(linear_cvfit, X_test, type='response', s=linear_cvfit$lambda.min)
#test_predictions
```

```{r}
vbc_test <- y_test$binding_counts
nbc_test <- (vbc - min(vbc_test))/(max(vbc_test) - min(vbc_test))
```

```{r}
plot(test_predictions, nbc_test)
```


```{r}
plot(train_predictions, nbc)
```
...
```{r}
foxa1_tracks <- enformer_annotations[enformer_annotations$target == 'FOXP3', ]$feature_names
```

```{r}
enformer_annotations[startsWith(enformer_annotations$target, 'FOX'), ]
```
...
```{r}
coeffs <- collate_coefficients(linear_cvfit)
```

```{r}
coeffs[coeffs[, 4] %in% foxa1_tracks, ]
```

```{r}
a <- coeffs[abs(coeffs[, 1]) > 0, ]$feature_names
a
```

```{r}
enformer_annotations[enformer_annotations$feature_names %in% a, ]
```

### Plotting

```{r}
foxa1_features <- data.frame(temp_dt)[, c('binding_counts', foxa1_tracks)]
foxa1_features[1:5, ]
```

```{r}
plot(foxa1_features$f_954, foxa1_features$binding_counts)
```

```{r}
cor.test(foxa1_features$f_3724, foxa1_features$binding_counts)
```

```{r}
apply(foxa1_features[, 2:ncol(foxa1_features)], 2, function(each_col){
  cor.test(each_col, foxa1_features[, 1])
})
```
...
```{r}
# path to the data
training_data <- glue("{project_dir}/data/enet_data/data_{data_date}/train_enet_{data_date}.csv.gz")
...
Read in the model(s)
```{r}
enet_binary_model <- readRDS(paste0(project_dir, '/models/enet_models/kawakami_center_binary_single_large_2022-12-22.rds'))
enet_binary_model
```
