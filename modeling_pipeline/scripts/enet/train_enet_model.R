...
unique_id <- arguments[2] #"large_batch" #
...
cv_model <- glmnet::cv.glmnet(x=X_train, y=y_train$class, family = "binomial", type.measure = "auc", alpha = 0.5, keep=T, parallel=T, nfolds=5, trace.it=T)
print(cv_model)
doParallel::stopImplicitCluster()

#names(enet_center_binary_models_list) <- mixing_parameters

print('[INFO] Finished with binary, single cv model\n')
...
print('[INFO] Saving the cv model')

# save the model
saveRDS(cv_model, file=glue('{project_dir}/models/enet_models/{id}_center_binary_{unique_id}_{run_date}.rds'))
