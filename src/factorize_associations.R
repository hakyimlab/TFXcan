# Author: Temi
# Description: NOT PART OF THE ACTIVE PIPELINE — kept as reference only. Not referenced by any
#   wrapper (.sbatch) in this directory; looks like an earlier draft that got forked into
#   prepare_summary_matrices.R (same option list, same z-ratio-matrix-building logic), which is
#   what the pipeline actually uses now. It also has a leftover block at the bottom that runs
#   flashier directly, but that block uses opt$batch/opt$priorL/opt$priorF/opt$greedy_Kmax which
#   aren't declared in option_list here — so running it as-is would fail.
# Usage: Rscript factorize_associations.R --gwas_summary_statistic <file> --tfxcan_summary_statistic <file> --output_basename <name> --output_directory <dir> --temporary_directory <dir> --number_resamples <int> --number_batches <int>
...
option_list <- list(
    ...
    make_option("--temporary_directory", help='[Input] Directory for intermediate files (random-subset batch rds/txt files)'),
...
#======== This block runs flashier on all the subsets ========
# UNSURE: this block references opt$batch, opt$priorL, opt$priorF, opt$greedy_Kmax, none of which
# are declared in option_list above — as written this would hit choosePrior(NULL) and error out.
# Looks unfinished/copy-pasted from repeat_flash.R.


what_extension <- tools::file_ext(glue("{opt$output_directory}/{opt$output_basename}.zratios.matrices.rds"))
if(what_extension %in% c('rds', 'rds.gz')){
    print(glue('INFO - Reading data from {opt$data}'))
    dt <- readRDS(glue("{opt$output_directory}/{opt$output_basename}.zratios.matrices.rds"))
} 

splits <- glue::glue('{consensus_subsets_directory}/{opt$output_basename}.random_subsets.{}.rds')
what_extension <- tools::file_ext(splits)
if(what_extension %in% c('rds', 'rds.gz')){
    sp <- readRDS(splits)[[1]]
