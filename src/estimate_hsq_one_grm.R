...
grm_file <- glue("/project2/haky/Data/baca_cwas/vcfs/hg38/merged_files/grm/baca_individuals.plink.merged.grm")
...
print(glue("INFO - dimension of the GRM: {nrow(chrgrm)} by {ncol(chrgrm)}"))
print(glue("INFO - dimension of the Ymat: {nrow(Ymat)} by {ncol(Ymat)}"))
...
chrhsq <- apply(Ymat, 2, function(Yarbs){
    ...
    tryCatch({
        ...
        message(glue("Error in calculating hsq for {colnames(Ymat)}: {e}"))
        oo <- rep(NA, 4) |> as.matrix()
        rownames(oo) <- c("maximum", "objective", "se", "pval")
        return(oo)
    }, warning = function(w){
        message(glue("Warning in calculating hsq for {colnames(Ymat)}: {w}"))
        oo <- rep(NA, 4) |> as.matrix()
        rownames(oo) <- c("maximum", "objective", "se", "pval")
        return(oo)
    })
...
# colbind
xout <- do.call('cbind', chrhsq) |> t()
rownames(xout) <- colnames(Ymat)

# use abind to create a 3D array
# out <- abind::abind(out, along = 0)

data.table::fwrite(xout, file = glue("/beagle3/haky/users/temi/projects/Enpact/experiments/heritability/per_arbs_full_grm_hsq_estimate.txt.gz"), sep = "\t", row.names = TRUE, col.names = TRUE, compress = "gzip")

# saveRDS(xout, file = glue("/beagle3/haky/users/temi/projects/Enpact/experiments/heritability/per_arbs_full_grm_hsq_estimate.rds.gz"), compress = TRUE)
