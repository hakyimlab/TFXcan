...
performance_file <- glue::glue("{tfxcan_project_directory}/data/enpact/performance/ENPACT_734_2025-04-24.compiled_stats.with_auprc.tsv")
...
```{r}
ggsave(glue::glue('{figures_directory}/qqplot_other_examples.png'), qqplot_other_examples, width = 8000, height = 5000, dpi = 800, units = 'px')
```
...
### Performance of incremental AR models across the genome: auroc
#### auroc vs number of training data for AR:Prostate
```{r}
...
auroc_206 <- base::subset(test_performance, model == "AR_Prostate")$auroc
...
incremental_samples_performance_barplot <- incremental_test_performance %>%
    ...
    geom_hline(yintercept = auroc_206, col = 'grey', lty = 'dashed')
...
```{r}
ggplot2::ggsave(glue::glue('{figures_directory}/suppl.figure1A.png'), incremental_samples_performance_barplot, width = 6000, height = 3000, dpi = 800, units = 'px')
...
predictions <- lapply(tfs_tissues, function(each_tft){
  one_ex <- glue('/beagle3/haky/users/temi/projects/Enpact/data/enpact/training/evaluations/{each_tft}_{rundate}.logistic.test_eval.txt.gz') #|> Sys.glob()
...
```{r}
ggsave(file.path(glue::glue("{figures_directory}/suppl.figure1D.png")), pioneer_weights_plot, width = 4000, height = 4000, units = 'px', dpi = 800)
```

```{r}
SUPPLEMENTARY_FIGURE_1$E <- pioneer_weights_plot
```
...
```{r}
ggsave(file.path(glue::glue("{figures_directory}/suppl.figure1C.png")), tissue_enrichment_plot, width = 5000, height = 5000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(glue::glue("{figures_directory}/figure2.png")), f2, width = 6000, height = 5000, units = 'px', dpi = 800)
```
...
# COMBine D and E first
plot_de <- patchwork::wrap_plots(list = suppfigure1[c('D', 'E')], ncol = 2) +
    plot_layout(widths = c(1, 0.5), heights = c(1, 0.4), 
        design = "AB
                  A#") +
    plot_annotation(tag_levels = list(c('D', 'E')), 
        theme = theme(plot.tag = element_text(size = 10.6, family = 'Helvetica', face = 'plain')))

# ggsave(file.path('/beagle3/haky/users/temi/projects/Enpact/figures/suppl.figure1DE.png'), plot_de, width = 8000, height = 5000, units = 'px', dpi = 800)

# $A, suppfigure1$B, suppfigure1$C, suppfigure1$D, suppfigure1$E
supp1_plot <- patchwork::wrap_plots(suppfigure1$A, suppfigure1$B, suppfigure1$C, plot_de, ncol = 1) +
    plot_layout(heights = c(0.3, 0.5, 0.4, 0.5), design = 
             'AA
              BB
              CC
              DD') +
...
ggsave(file.path(glue::glue("{figures_directory}/suppl.figure1ABCDE.png")), supp1_plot, width = 8000, height = 15000, units = 'px', dpi = 800)
```
...
ggsave(file.path(figures_directory, "suppl.figure2A.png"), scatterplot_top12_enpact_observed, width = 8000, height = 6000, units = 'px', dpi = 800)
```
...
```{r}
cor_enp_cwas <- data.table::fread(file.path(tfxcan_project_directory, 'data/cwas/observed', 'baca.enpact_cwas_correlations.txt.gz')) %>% dplyr::filter(!is.na(pvalue))
cor_null <- data.table::fread(file.path(tfxcan_project_directory, 'data/cwas/observed', 'baca.null_correlations.txt.gz')) %>% dplyr::filter(!is.na(pvalue))
```
...
ggsave(file.path(figures_directory, 'suppl.figure2B.png'), scatterplot_top12_enpact_cwas, width = 8000, height = 6000, units = 'px', dpi = 800)
```
...
ttplot <- top12_ect %>%
    ...
    geom_text(data=tt, mapping=aes(x = -Inf, y = Inf, label = label, col = 'red'), hjust = -0.1, vjust = 2, lineheight = 1, size = 2.5)
#  
ggsave(file.path(figures_directory, 'logits.figure3F.png'), ttplot, width = 3000, height = 3000, units = 'px', dpi = 800)
```
...
```{r, fig.height=6, fig.width=9}
ggsave(file.path(figures_directory, "figure3.png"), f3, width = 8000, height = 5500, units = "px", dpi = 800)
...
```{r}
SUPPLEMENTARY_FIGURE_2 <- lapply(SUPPLEMENTARY_FIGURE_2, ggplotify::as.ggplot)
```

```{r}
suppfigure2 <- rlang::duplicate(SUPPLEMENTARY_FIGURE_2)
names(suppfigure2)
```

```{r}
# $A, suppfigure1$B, suppfigure1$C, suppfigure1$D, suppfigure1$E
supp2_plot <- patchwork::wrap_plots(list = suppfigure2[c('A', 'B')], ncol = 1) +
    plot_layout(heights = c(1,1), design = 
             'AA
              BB') +
    plot_annotation(tag_levels = list(c('A', 'B')), 
        theme = theme(plot.tag = element_text(size = 10.6, family = 'Helvetica', face = 'plain')))

ggsave(file.path(figures_directory, 'suppl.figure2AB.png'), supp2_plot, width = 8000, height = 10000, units = 'px', dpi = 800)
```
...
```{r}
SUPPLEMENTARY_FIGURE_3$A <- histogram_nsnps_used
ggsave(file.path(figures_directory, 'suppl.figure3A.png'), histogram_fracsnps_used, width = 3000, height = 3000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(figures_directory,"figure4.png"), f4, width = 6000, height = 2000, units = "px", dpi = 800)
...
```{r}
SUPPLEMENTARY_FIGURE_3$B <- peak_center_distances_distribution
ggsave(file.path(figures_directory, "suppl.figure3B.png"), peak_center_distances_distribution, width = 8000, height = 3000, units = 'px', dpi = 800)
```


### Save supplementary figure 3
```{r}
SUPPLEMENTARY_FIGURE_3 <- lapply(SUPPLEMENTARY_FIGURE_3, ggplotify::as.ggplot)
```

```{r}
suppfigure3 <- rlang::duplicate(SUPPLEMENTARY_FIGURE_3)
names(suppfigure3)
```

```{r}
# $A, suppfigure1$B, suppfigure1$C, suppfigure1$D, suppfigure1$E
supp3_plot <- patchwork::wrap_plots(list = suppfigure3[c('A', 'B')], ncol = 2) +
    plot_layout(widths = c(0.5,1), design = 
             'ABB') +
    plot_annotation(tag_levels = list(c('A', 'B')), 
        theme = theme(plot.tag = element_text(size = 10.6, family = 'Helvetica', face = 'plain')))

ggsave(file.path(figures_directory, 'suppl.figure3AB.png'), supp3_plot, width = 9000, height = 2500, units = 'px', dpi = 800)
```
...
```{r}
FIGURE_5 <- list()
SUPPLEMENTARY_FIGURE_4 <- list()
```

```{r}
dt_gexpr <- data.table::fread(file.path(metadata_directory, 'prostate_cancer_risk.gexpr.spredixcan.txt.gz'))
...
cwas_74 <- data.table::fread(file.path(metadata_directory, 'CWAS_significant_prostate_cancer_risk_associations.top74.txt.gz'))
...
SUPPLEMENTARY_FIGURE_4$A <- ldblock_distribution
```
...
```{r}
ggsave(file.path(figures_directory, "suppl.figure4A.png"), ldblock_distribution, width = 10000, height = 4000, units = 'px', dpi = 800)
```
...
cwas_5580 <- data.table::fread(file.path(metadata_directory, 'CWAS_significant_prostate_cancer_risk_associations.top5580.txt.gz'))
cwas_5580
```
...
png(filename=file.path(figures_directory, "figure5.png"), width = 8000, height = 4000, units = "px", res = 800)
f5
dev.off()
...
png(file.path(figures_directory, "suppl.figure4B.png"), width = 8000, height = 4000, res = 800, units = 'px')
...
SUPPLEMENTARY_FIGURE_4$B <- locus_zoom_plots
```

### Save supplementary figure 4
```{r}
SUPPLEMENTARY_FIGURE_4 <- lapply(SUPPLEMENTARY_FIGURE_4, ggplotify::as.ggplot)
```

```{r}
suppfigure4 <- rlang::duplicate(SUPPLEMENTARY_FIGURE_4)
names(suppfigure4)
```

```{r}
# $A, suppfigure1$B, suppfigure1$C, suppfigure1$D, suppfigure1$E
supp4_plot <- patchwork::wrap_plots(list = suppfigure4[c('A', 'B')], ncol = 2) +
    plot_layout(widths = c(0.6,1), design = 
             'ABBB') +
    plot_annotation(tag_levels = list(c('A', 'B')), 
        theme = theme(plot.tag = element_text(size = 10.6, family = 'Helvetica', face = 'plain')))

ggsave(file.path(figures_directory, 'suppl.figure4AB.png'), supp4_plot, width = 18000, height = 7000, units = 'px', dpi = 800)
```
