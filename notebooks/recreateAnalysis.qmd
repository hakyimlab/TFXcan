---
title: TFXcan paper analysis (main and supplementary)
date: "2024-11-21"
date-modified: today
author: Temi
fig-format: png
...
```{r}
renv::init(project = '/beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-project')
renv::status(project = '/beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-project')

renv::install("glue", project = '/beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-project')

renv::hydrate(packages = c('glue'), project = '/beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-project', 
             library = '/beagle3/haky/users/temi/software/conda_envs/renviron/lib/R/library')

renv::snapshot(lockfile = '/beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-project/renv.lock', library = '/beagle3/haky/users/temi/software/conda_envs/renviron/lib/R/library')


conda env export | grep -v "^prefix: " | grep -v "^name: " > /beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-environment.yaml
sed '/linux/d; /mutex/d; /gcc/d; /fortran/d; /pixman/d; /lib/d; /icu/d; /zlib/d; /bzip2/d; /lzma/d; /readline/d; /ncurses/d; /cairo/d; /fontconfig/d; /freetype/d; /glibc/d' /beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-environment.yaml > /beagle3/haky/users/temi/projects/TFXcan/software/tfxcan-environment.mac.yaml
```
...
Here is an environment yaml file you can use: `https://github.com/hakyimlab/TFXcan/blob/master/software/tfxcan-environment.yaml`
Run the following:

1. conda env create --file ./tfxcan-environment.yaml --name tfxcan-environment #or a name you want

2. conda activate tfxcan-environment

3. r #or R or radian
:::

::: {.callout-warning}
There will be issues if you try to build the environment on a mac. This is because this environment was built on a linux platform, and cross-platform compatibility is not guaranteed. If you are on a mac, you can try to install the packages manually or use the `renv` package to create a virtual environment for this notebook.
:::
...
## if using a mounted box folder and have access to the imlab box

```{r}
# uncomment what you need here

# PRE = '/Users/temi/Library/CloudStorage/Box-Box/imlab-data'
# PRE = '/Users/temidayoadeluwa/Library/CloudStorage/Box-Box/imlab-data'
# PRE = "/Users/haekyungim/Library/CloudStorage/Box-Box/LargeFiles/imlab-data"

analysis_directory <- file.path(PRE, 'TFXcan/analysis')
output_directory <- file.path(analysis_directory, 'output')
if(!dir.exists(output_directory)) {
    dir.create(output_directory, recursive = TRUE)
}
```

## if you need to download the data to your local machine
The data used in this notebook are here on Box:
https://uchicago.box.com/s/p07onnhc6u4kojrqs39otrcrsp8qk8hs

When you have the data downloaded, you can set the `analysis_directory` variable to the path where you saved the data.
...
```{r}
# create a new directory for the output and figures generated in this notebook
output_directory <- file.path(analysis_directory, 'output')
if(!dir.exists(output_directory)) {
    dir.create(output_directory, recursive = TRUE)
}
...
# Load libraries and functions, and set some variables

## Initial definitions library loading
```{r initial definitions, message=F, warning=F}
...
devtools::source_gist('https://gist.github.com/TemiPete/73751e3b6695808065832b1950fc3856')
```
...
```{r}
# mappings
dt_mapping <- data.table::fread(file.path(analysis_directory, 'files', 'enformer-tracks-annotated-resaved-2.txt')) %>%
...
dt_slims <- data.table::fread(file.path(analysis_directory, 'files', "enpact_slims.txt")) %>%
...
performance_file <- file.path(analysis_directory, 'data/enpact/weights', 'ENPACT_734_2025-04-24.compiled_stats.txt')
...
```{r}
weights_file <- file.path(analysis_directory, 'data/enpact/weights', 'ENPACT_734_2025-04-24.compiled_weights.lambda.1se.txt.gz')
...
```{r}
ggsave(file.path(output_directory, 'qqplot_all_weights.png'), qqplot_all_weights, width = 4000, height = 3000, dpi = 800, units = 'px')
```
...
```{r}
ggsave(file.path(output_directory, 'qqplot_ar_prostate_weights.png'), qqplot_ar_prostate_weights, width = 4000, height = 3000, dpi = 800, units = 'px')
```
...
```{r}
ggsave(file.path(output_directory, 'qqplot_other_examples.png'), qqplot_other_examples, width = 8000, height = 5000, dpi = 800, units = 'px')
```
...
```{r}
cistrome_db <- data.table::fread(file.path(analysis_directory, 'files', "human_factor_full_QC.txt")) %>%
...
```{r}
incremental_experiment_summary <- data.table::fread(file.path(analysis_directory, 'data/tests/incremental50', 'ENPACT_incremental50_2025-04-26.compiled_stats.txt'))
...
```{r}
ggplot2::ggsave(file.path(output_directory, 'suppl.figure1A.png'), incremental_samples_performance_barplot, width = 6000, height = 3000, dpi = 800, units = 'px')
...
predictions <- lapply(tfs_tissues, function(each_tft){
  one_ex <- glue('{analysis_directory}/data/enpact/evaluations/{each_tft}_{rundate}.logistic.test_eval.txt.gz') #|> Sys.glob()
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure1D.png'), pw_plot, width = 4000, height = 4000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure1C.png'), tissue_enrichment_plot, width = 7000, height = 7000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure1AB.png'), top_plot, width = 10000, height = 6000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(output_directory, 'figure2.png'), f2, width = 8000, height = 2500, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure1.png'), s1, width = 10000, height = 20000, units = 'px', dpi = 800)
```
...
enpact_mat <- data.table::fread(file.path(analysis_directory, 'data/predictions', 'baca.enpact_scores.AR_Prostate.txt.gz')) %>% 
    tibble::column_to_rownames("V1") %>%
    as.matrix()

gt_mat_binarized <- data.table::fread(file.path(analysis_directory, 'data/predictions', 'baca.observed_binding.binarized.txt.gz')) %>%
    tibble::column_to_rownames("V1") %>%
    as.matrix()

cwas_mat <- data.table::fread(file.path(analysis_directory, 'data/predictions', 'baca.cwas_scores.AR_Prostate.txt.gz')) %>%
    tibble::column_to_rownames("V1") %>%
    as.matrix()
```
...
```{r}
dt_enpact_observed <- data.table::fread(file.path(analysis_directory, 'data/tests', 'baca.enpact_observed_ttest.txt.gz')) %>% dplyr::filter(!is.na(pval))
dt_cwas_observed <- data.table::fread(file.path(analysis_directory, 'data/tests', 'baca.cwas_observed_ttest.txt.gz')) %>% dplyr::filter(!is.na(pval))
```
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure2A.png'), scatterplot_top12_enpact_observed, width = 8000, height = 6000, units = 'px', dpi = 800)
```
...
```{r}
cor_enp_cwas <- data.table::fread(file.path(analysis_directory, 'data/tests', 'baca.enpact_cwas_correlations.txt.gz')) %>% dplyr::filter(!is.na(pvalue))
cor_null <- data.table::fread(file.path(analysis_directory, 'data/tests', 'baca.null_correlations.txt.gz')) %>% dplyr::filter(!is.na(pvalue))
```
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure2B.png'), scatterplot_top12_enpact_cwas, width = 8000, height = 6000, units = 'px', dpi = 800)
```
...
ttplot <- top12_ect %>%
    ...
    geom_text(data=tt, mapping=aes(x = -Inf, y = Inf, label = label, col = 'red'), hjust = -0.1, vjust = 2, lineheight = 1, size = 2.5)
#  
ggsave(file.path(output_directory, 'logits.figure3F.png'), ttplot, width = 3000, height = 3000, units = 'px', dpi = 800)
```
...
```{r, fig.height=6, fig.width=9}
ggsave(file.path(output_directory, "figure3.png"), f3, width = 8000, height = 5500, units = "px", dpi = 800)
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure3A.png'), histogram_fracsnps_used, width = 3000, height = 3000, units = 'px', dpi = 800)
```
...
```{r}
hg38_maps <- data.table::fread(file.path(analysis_directory, 'files', 'baca_cwas_loci_hg38.bed')) 
hg38_maps <- setnames(hg38_maps, c('chr', 'start', 'end', 'hg19_locus')) %>%
    tidyr::unite('hg38_locus', chr:end, sep='_')
cwas_models <- readxl::read_excel(file.path(analysis_directory, 'files', 'cwas_supplementary_tables.xlsx'), sheet = 'Table S3', skip = 1) %>%
...
```{r}
distance_from_center <- data.table::fread(file.path(analysis_directory, 'files', 'snp_weights_distance.txt.gz'))
```
...
```{r}
ggsave(file.path(output_directory, "figure4.png"), f4, width = 6000, height = 2000, units = "px", dpi = 800)
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure3B.png'), peak_center_distances_distribution, width = 8000, height = 3000, units = 'px', dpi = 800)
```
...
```{r}
dt_gexpr <- data.table::fread(file.path(analysis_directory, 'data/twas', 'prostate_cancer_risk.gexpr.spredixcan.txt.gz'))
dt_tfxcan <- data.table::fread(file.path(analysis_directory, 'data/tfxcan', 'AR_Prostate.logistic.TFXcan.prostate_cancer_risk.csv'))
cwas_74 <- data.table::fread(file.path(analysis_directory, 'files', 'CWAS_significant_prostate_cancer_risk_associations.top74.txt.gz'))
ldblocks <- data.table::fread(file.path(analysis_directory, 'files', 'hg38_fourier_ls-all.bed'), header=F, col.names=c('chrom', 'start', 'end'))
```
...
```{r}
ggsave(file.path(output_directory, 'suppl.figure4A.png'), ldblock_distribution, width = 10000, height = 4000, units = 'px', dpi = 800)
```
...
cwas_5580 <- data.table::fread(file.path(analysis_directory, 'files', 'CWAS_significant_prostate_cancer_risk_associations.top5580.txt.gz'))
cwas_5580
```
...
```{r}
cwas_5580 <- readxl::read_excel(file.path(analysis_directory, 'files', 'cwas_supplementary_tables.xlsx'), sheet = 'Table S3', skip = 1) %>%
...
png(filename=file.path(output_directory, "figure5.png"), width = 8000, height = 4000, units = "px", res = 800)
f5
dev.off()
...
```{r}
tfx %>% cat(sep='\n', file=file.path(output_directory, 'tfxcan_genes.exclusive.txt'))
cwa %>% cat(sep='\n', file=file.path(output_directory, 'cwas_genes.exclusive.txt'))
com %>% cat(sep='\n', file=file.path(output_directory, 'common_genes.exclusive.txt'))
all_g %>% cat(sep='\n', file=file.path(output_directory, 'all_genes.exclusive.txt'))
```

### Gene set overrepresentation analysis with gprofiler

```{r}
tfxcan_genes <- readLines(file.path(output_directory, 'tfxcan_genes.exclusive.txt'))
cwas_genes <- readLines(file.path(output_directory, 'cwas_genes.exclusive.txt'))
common_genes <- readLines(file.path(output_directory, 'common_genes.exclusive.txt'))
all_genes <- readLines(file.path(output_directory, 'all_genes.exclusive.txt'))
```
...
```{r}
ggsave(file.path(output_directory, "suppl.figure4C.png"), goverrep_plot, width = 11000, height = 3500, units = "px", dpi = 800)
```
...
```{r}
# save the plot
ggsave(filename=file.path(output_directory, "suppl.figure4D.png"), locus_zoom_plots, width = 12000, height = 12000, units = "px", dpi = 800)
```
...
```{r}
ggsave(filename=file.path(output_directory, "suppl.figure6A.png"), tfxcan_tf_pval_ranking, width = 5000, height = 15000, units = "px", dpi = 800)
```
...
```{r}
ggsave(filename=file.path(output_directory, "suppl.figure6B.png"), tfxcan_tissue_pval_ranking, width = 5000, height = 8000, units = "px", dpi = 800)
```
...
data_list <- readRDS(file.path(analysis_directory, "data/tenerife/PrCa.tfxcan.zscores.matrices.rds"))
```
...
ggsave(filename=file.path(output_directory, "prca_full_tenerife_plot.pdf"), full_tenerife, width = 50000, height = 20000, units = "px", dpi = 800, limitsize = FALSE)
```
...
```{r}
zratio_matdev <- readRDS(file.path(analysis_directory, "data/tenerife/PrCa.tfxcan.zratios.matrix.rds"))
```
...
```{r}
ggsave(file.path(output_directory, "figure6A.png"), tenerife_plots, width = 6000, height = 3000, units = 'px', dpi = 800)
```

### Matrix factorization of z-scores

```{r}
zflash <- readRDS(file.path(analysis_directory, "data/tenerife/PrCa.tfxcan.zratios.flash.rds"))
```
...
```{r}
ggsave(file.path(output_directory, "suppl.figure6CD.png"), variance_explained, width = 4000, height = 4000, units = 'px', dpi = 800)
```
...
```{r}
outfile <- file.path(analysis_directory, 'data/consensus', 'prca_risk.program_clusters.1000samples.L2_norm.txt.gz')
mat_file <- file.path(analysis_directory, 'data/consensus', 'prca_risk.programs_matrix.1000samples.L2_norm.rds.gz')
corr_file <- file.path(analysis_directory, 'data/consensus', 'prca_risk.programs_corrmatrix.1000samples.L2_norm.rds.gz')
```
...
cluster_scores <- data.table::fread(file.path(analysis_directory, 'data/consensus', 'prca_risk.programs_clara_silhouette.1000samples.L2_norm.txt.gz'))
```
...
```{r}
ggsave(filename=file.path(output_directory, "suppl.figure6E.png"), optimal_num, width = 3000, height = 3000, units = "px", dpi = 800)
```
...
```{r}
ggsave(filename=file.path(output_directory, "suppl.figure6F.png"), num_inferred_programs_per_subsample, width = 3000, height = 4000, units = "px", dpi = 800)
```
...
ggsave(file.path(output_directory, "suppl.figure6AB.png"), x6a, width = 7000, height = 15000, units = 'px', dpi = 800)
```
...
```
```{r}
ggsave(file.path(output_directory, "suppl.figure6CDEFG.png"), x6, width = 8000, height = 4000, units = 'px', dpi = 800)
```
...
If there is a need to modify the programs and the colors and all that, you can edit this section. Otherwise, just read it in. 
...
# data.table::fwrite(program_meta, file.path(analysis_directory, 'files/prca.programs.metadata.txt'), sep = '\t', quote = F, col.names = T, row.names = F)

```

```{r}
program_meta <- data.table::fread(file.path(analysis_directory, 'files/prca.programs.metadata.txt'))
program_meta
```
...
```{r}
ggsave(file.path(output_directory, "subsamples.cluster1000.png"), hmap, width = 18000, height = 18000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(output_directory, "subsamples.programs.cluster1000.png"), hmap_programs, width = 12000, height = 12000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(output_directory, "suppl.figure6H.png"), num_program_assigned, width = 10000, height = 4000, units = 'px', dpi = 800)
```
...
ggsave(file.path(output_directory, "figure6B.png"), program_plots, width = 18000, height = 2000, units = 'px', dpi = 800)
```
...
```{r}
data.table::fwrite(pmats, file = file.path(output_directory, 'prca.tf.clusters.txt'), row.names = TRUE, col.names = TRUE, sep = '\t', quote = F)
```
...
```{r}
ggsave(file.path(output_directory, "suppl.figure6I.png"), histogram_avg_contributions, width = 8000, height = 8000, units = 'px', dpi = 800)
```
...
nuc_recep <- data.table::fread(file.path(analysis_directory, 'files', 'nuclear_receptors.txt'), fill=FALSE)
...
```{r}
loci_assignment <- file.path(analysis_directory, 'data/consensus/prca_risk.loci_assignments.1000samples.L2_norm.txt.gz')
```
...
```{r}
data.table::fwrite(lmats, file = file.path(output_directory, 'prca.loci.assignments.txt'), row.names = TRUE, col.names = TRUE, sep = '\t', quote = F)
```
...
```{r}
mandt <- data.table::fread(file.path(analysis_directory, "files/tfxcan.prca_gwas.txt.gz"), sep = '\t', quote = F, header = T)
```
...
```{r}
ggsave(file.path(output_directory, 'figure6C.png'), man.g.colors, width = 15000, height = 5000, dpi = 800, units = 'px')
```
...
```{r}
#save f6
ggsave(file.path(output_directory, 'figure6.png'), f6, width = 23000, height = 10000, dpi = 800, units = 'px')
...
```{r}
experiments_threshold_pass <- data.table::fread(file.path(analysis_directory, 'human_factor_full_QC.txt')) %>%
...
This section counts the number of peaks per individual in the Baca et al. 2023 data, which is used for the CWAS analysis. I did not provide the data here. 
```{r, eval = F}
...
# Save workbook
openxlsx::saveWorkbook(wb, file.path(output_directory, 'enpact-paper.supplementary_tables.xlsx'), overwrite = TRUE)
```
