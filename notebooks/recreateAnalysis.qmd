...
```{r}
knitr::opts_chunk$set(cache.rebuid = FALSE, fig.width = 4, fig.height = 4, fig.format = 'png', fig.dpi = 800, cache = TRUE)
```
...
SUPPLEMENTARY_TABLES[[length(SUPPLEMENTARY_TABLES) + 1]] <- perf.disc_power
```
...
ggsave(file.path(figures_directory, "SF1.discriminatory_power.AR-Prostate.open_region.png"), pp.boxplot.open, width = 3000, height = 3000, dpi = 800, units = 'px')

ggsave(file.path(figures_directory, "SF1.median_differences.TF-Tissues.open_region.png"), pp1,  width = 3000, height = 3000, dpi = 800, units = 'px')

ggsave(file.path(figures_directory, "SF1.qqplot_pvalues.TF-Tissues.open_region.png"), pp2, width = 3000, height = 3000, dpi = 800, units = 'px')
```
...
```{r}
ggsave(file.path(figures_directory, "SF1.binding_vs_accessibility.AR-Prostate.open_region.png"), pp.points.binding_accessibility, width = 3000, height = 3000, dpi = 800, units = 'px')

ggsave(file.path(figures_directory, "SF1.accessibility_scores.AR-Prostate.open_region.png"), pp.boxplot.accessibility, width = 3000, height = 3000, dpi = 800, units = 'px')
```
...
```{r}
ggsave(glue::glue('{figures_directory}/SF1.qqplot_other_examples.png'), qqplot_other_examples, width = 8000, height = 5000, dpi = 800, units = 'px')
```
...
ggsave(glue("{figures_directory}/SF1.test_auprc_vs_num_bound.png"), auprc_num_bound, width = 5000, height = 4000, dpi = 800, units = 'px')
...
```{r}
SUPPLEMENTARY_TABLES[[length(SUPPLEMENTARY_TABLES) + 1]] <- dt_pioneer[,1] %>% rename(pioneer_tf = known)
```
...
```{r}
ggsave(file.path(glue::glue("{figures_directory}/SF1.pioneer_tfs_weight_plot.png")), pioneer_weights_plot, width = 3000, height = 3000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(glue::glue("{figures_directory}/SF1.tissue_enrichment_plot.png")), tissue_enrichment_plot, width = 5000, height = 5000, units = 'px', dpi = 800)
```
...
ggsave(file.path(figures_directory, "SF2.cwas_obs.spearman_pval.qqplot.png"), qqplot_cwas_obs_pvalues, width = 3000, height = 3000, units = 'px', dpi = 800)
...
gencor.signif <- dt_merged.null_obs %>%
    dplyr::mutate(fdr = qvalue::qvalue(pvalue.obs, fdr.level = 0.05)$significant)
...
```{r}
1 - with(gencor.signif, qvalue::qvalue(pvalue.obs))$pi0
```
...
```{r}
SUPPLEMENTARY_TABLES[[length(SUPPLEMENTARY_TABLES) + 1]] <- dt_merged.null_obs %>% dplyr::rename(arbs = locus) %>% dplyr::select(-comparison.null, -comparison.obs)
```
...
ggsave(file.path(figures_directory, 'SF2.top12_enpact_cwas.png'), scatterplot_top12_enpact_cwas, width = 8000, height = 6000, units = 'px', dpi = 800)
```
...
```{r}
ggsave(file.path(figures_directory, 'SF3.nsnps_used.AR_Prostate.png'), histogram_fracsnps_used, width = 3000, height = 3000, units = 'px', dpi = 800)
```
...
## Is SNP-Enpact similar to CWAS top QTLs?
...
top1_qtl_predictions <- data.table::fread(file.path(tfxcan_project_directory, 'experiments/cwas_scores/output/top1.hg38/baca_cwas_predict.txt')) %>% dplyr::select(-FID)
...
plot_corr <- ggplot() +
    ...
    theme(legend.position = "top", legend.position.inside = c(0.2, 0.7), legend.key = element_rect(fill = "transparent", colour = NA))
...
plot_pvalue <- qqcompare.ggplot(result$pvalue, null_result$pvalue, names = c('observed', 'null')) +
    ...
    theme(legend.position = "top", legend.position.inside = c(0.7, 0.7), legend.key = element_rect(fill = "transparent", colour = NA))
...
```{r}
summary(result$r); summary(abs(result$r)); summary((result$r)^2); prop.table(table(abs(result$r) > 0.5))
```
...
f4 <- patchwork::wrap_plots(figure_4, ncol = 3) + plot_annotation(tag_levels = list(c('A', 'B', 'C', 'D', 'E', 'F'))) & theme(plot.tag = element_text(size = 9.33, family = 'Helvetica'))
```

```{r}
ggsave(file.path(figures_directory,"figure4.png"), f4, width = 6000, height = 4000, units = "px", dpi = 800)
...
```{r}
ggsave(file.path(figures_directory, "SF4.ldblock_distribution.png"), ldblock_distribution, width = 10000, height = 6000, units = 'px', dpi = 800)
```
...
ggsave(file.path(figures_directory, 'SF4.locus_zoom_plots.png'), locus_zoom_plots, width = 18000, height = 12000, units = 'px', dpi = 800)
```
...
ggsave(glue("{figures_directory}/SF4.enrichment_analysis_genes_near_44_distinct_ARBS.png"), pp, width = 5500, height = 6000, dpi = 800, units = 'px')
...
ggsave(glue("{figures_directory}/SF4.fdr.enrichment_analysis_genes_near_44_distinct_ARBS.png"), pp, width = 5500, height = 2000, dpi = 800, units = 'px')
...
```{r}
ggsave(filename=file.path(figures_directory, "SF6.tfxcan_tf_pval_ranking.png"), tfxcan_tf_pval_ranking, width = 5000, height = 15000, units = "px", dpi = 800)
```
...
```{r}
ggsave(filename=file.path(figures_directory, "SF6.tfxcan_tissue_pval_ranking.png"), tfxcan_tf_pval_ranking, width = 5000, height = 15000, units = "px", dpi = 800)
```
...
ggsave(filename=file.path(figures_directory, "suppl.figure5.pdf"), full_tenerife, width = 50000, height = 20000, units = "px", dpi = 800, limitsize = FALSE)
```
...
```{r}
ggsave(filename=file.path(figures_directory, "SF6.optimal_num.png"), optimal_num, width = 3000, height = 3000, units = "px", dpi = 800)
```
...
```{r}
ggsave(filename=file.path(figures_directory, "SF6.num_inferred_programs_per_subsample.png"), num_inferred_programs_per_subsample, width = 3000, height = 4000, units = "px", dpi = 800)
```
...
#c('Liver', 'Program 1', 'REST', 'Program 2', 'Bone marrow', 'Blood', 'Hormone receptors', 'CTCF', 'Colon', 'Program 3', 'Program 4')
...
```{r}
ggsave(file.path(figures_directory, "SF6.subsamples.cluster1000.png"), hmap, width = 18000, height = 18000, units = 'px', dpi = 800)
```
...
nps <- purrr::map(1:length(program_names), function(i){
    X <- select_ks(cluster_assignments, i, yclust, samp = FALSE)
    ncol(X)
}, .progress = TRUE) %>% unlist() %>% setNames(paste0('cluster', 1:length(program_names))) 
...
```{r}
pclusts <- purrr::map(1:length(program_names), function(i){
    X <- select_ks(cluster_assignments, i, yclust, samp = FALSE)
    X <- rowMeans(X, na.rm = TRUE) |> as.matrix()
    return(X)
}, .progress = TRUE) %>% setNames(paste0('cluster', 1:length(program_names)))
```
...
program_meta <- program_meta %>%
    dplyr::arrange(match(name, c('Hormone receptors', 
        'Liver', 'Blood', 'Bone marrow', 'Colon', 'CTCF', 'REST', 'Program 1', 'Program 2')))
program_meta
```
```{r}
...
names(av_pps) <- names(pclusts)
```

```{r}
# av_pps$`Hormone receptors`
```
...
ggsave(file.path(figures_directory, "figure6B.png"), program_plots, width = 7000, height = 10000, units = 'px', dpi = 800)
```
...
colnames(pmats) <- paste0('k', 1:length(program_names))
...
```{r}
SUPPLEMENTARY_TABLES[[length(SUPPLEMENTARY_TABLES) + 1]] <- pmats
```

```{r}
data.table::fwrite(pmats, file = file.path(files_directory, 'prca.tf.clusters.txt'), row.names = TRUE, col.names = TRUE, sep = '\t', quote = F)
```
...
```{r}
SUPPLEMENTARY_TABLES[[length(SUPPLEMENTARY_TABLES) + 1]] <- lmats
```

```{r}
data.table::fwrite(lmats, file = file.path(files_directory, 'prca.loci.assignments.txt'), row.names = TRUE, col.names = TRUE, sep = '\t', quote = F)
```
...
```{r}
mandt <- data.table::fread(file.path(metadata_directory, "tfxcan.prca_gwas.txt.gz"), sep = '\t', quote = F, header = T)
```
...
```{r}
names_to_show <- c('Hormone receptors', 'Liver', 'Bone marrow', 'REST')
```

```{r, fig.width = 19, fig.height = 4}
main_programs <- patchwork::wrap_plots(plotlist = av_pps[names_to_show], nrow = 1, axes = 'collect') +
    patchwork::plot_annotation(title = '')

supplementary_programs <- patchwork::wrap_plots(plotlist = av_pps[!names(av_pps) %in% names_to_show], nrow = 2, axes = 'collect') +
    patchwork::plot_annotation(title = '')


FIGURE_6$program_plots <- main_programs
```
...
```{r}
f6 <- patchwork::wrap_plots(figure_6$tenerife_plots, figure_6$program_plots, figure_6$annotated_manhattan, nrow = 3) + plot_layout(heights = c(0.5, 0.3, 0.4), design = 
             'AA
              BB
              CC') +
...
ggsave(file.path(figures_directory, "figure6.png"), f6, width = 7500, height = 9000, dpi = 800, units = 'px')
# bottompanel <- figure_6$program_plots + plot_layout(nrow = 1)
```


```{r}
ggsave(file.path(figures_directory, 'SF6.other_prca_tfxcan_programs.png'), supplementary_programs, width = 7000, height = 3000, dpi = 800, units = 'px')
```
...
```{r}
#save f6
# ggsave('/beagle3/haky/users/temi/projects/Enpact/figures/figure6.png', f6, width = 10000, height = 20000, dpi = 800, units = 'px')
...
ggsave(file.path(figures_directory, "SF7.null-vs-true-interactions.networks_histograms.png"), null_int_plot.3, width = 13000, height = 10000, dpi = 800, units = 'px')
```
...
```{r}
ggsave(glue("{figures_directory}/SF7.enrichment_analysis_on_PROGRAM1.png"), enrichment_plots[['Program 1']], width = 5000, height = 2200, dpi = 800, units = 'px')

ggsave(glue("{figures_directory}/SF7.enrichment_analysis_on_PROGRAM2.png"), enrichment_plots[['Program 2']], width = 5000, height = 1500, dpi = 800, units = 'px')

ggsave(glue("{figures_directory}/SF7.enrichment_analysis_on_HORMONERECEPTORS.png"), enrichment_plots[['Hormone receptors']], width = 5000, height = 3500, dpi = 800, units = 'px')
