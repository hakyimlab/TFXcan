...
base_dir <- '/beagle3/haky/users/temi/projects/TFXcan/baca_cwas'
...
out <- purrr::map(.x=1:length(ar_files_locus), .f=function(i){
    ...
    load(glue('{output_dir}/{transcription_factor}/{locus}.wgt.RDat'), rdt <- new.env())
    ...
    dt <- merge(wgts, snp_info, by='snp_id') %>%
        dplyr::relocate(dt, all_of(c('locus', 'chr', 'position', 'a1', 'a2')), .after=snp_id)
    return(dt)
}, .progress=T)
