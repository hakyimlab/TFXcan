...
```{r}
fgt <- freedman_ground_truth[freedman_ground_truth$region %in% pscores_list$LuCaP_167$region, ]
fgt[1:5, ] ; dim(fgt)
```


```{r}
conds <- apply(fgt[, -c(1:3)], 1, function(each_r){
...
```{r}
boxplot(aggByCenter ~ class, data=pscores_dt, main='Predicted scores across all regions across all individuals', frame.plot=F, ylab='predicted scores')
```
...
pscores_pvalues_logreg <- purrr::map(.x=varying_classes$region, function(each_region){
    ...
    tgt <- glm(class ~ aggByCenter, data=pscores_ind_region, family='binomial') |> summary()
