...
todays_date <- data_date <- Sys.Date() #2023-01-06' #Sys.Date()
#TF <- 'GATA3'
```
...
```{r}
# predictor_file <- Sys.glob(glue('{project_dir}/motif_intervals/{dataset}/intervals_{data_date}/ground_truth/*.txt'))
# predictor_file
```
...
```{r}
rm(c('temp_dt', 'agg_transform_list'))
```
...
models_list <- purrr::map(.x=agg_methods, function(each_method){
    ...
    agg_rds <- glue('{model_dir}/{dataset}_{TF}_{each_method}_binary_{todays_date}.rds')
...
```{r}
models_coefficients <- purrr::map(.x=agg_methods, function(each_method){
    dt <- collate_coefficients(models_list[[each_method]])
    dt$agg_model <- each_method
    return(dt)
})
names(models_coefficients) <- agg_methods
```

```{r}
pdt <- models_coefficients$aggByPostCenter
pdt[1:10, ]
```
...
```{r}
temps <- purrr::map(.x=names(models_coefficients), function(each_aggm){
    plot_and_annotate_coefficients(models_coefficients[[each_aggm]], TF, each_aggm)
})
```

```{r}
ggpubr::ggarrange(plotlist=temps, common.legend=T, legend='top', nrow=length(temps))
```
...
cistrome_train_assess_metrics <- purrr::map(.x=agg_methods, function(each_method){
    ...
    a <- assess.glmnet(models_list[[each_method]], newx=train_data_list[[each_method]][ , -c(1:3)] |> as.matrix(), newy=train_data_list[[each_method]]$class, family='binomial')
...
```{r}
tm_train <- do.call('rbind', cistrome_train_assess_metrics)[, 'auc']
tm_train <- tm_train[order(tm_train,decreasing = TRUE)]
xbp <- barplot(tm_train, xaxt='n', space=0.1, xlab=NULL, ylab='AUC', col=ifelse(tm_train == max(tm_train),"green", ifelse(tm_train == min(tm_train), 'red', 'grey'))) #names.arg=names(tm_train), las=1.5)
text(cex=0.9, x=xbp+0.05, y=0.0, names(tm_train), xpd=TRUE, srt=25, pos=2)
mtext(glue('AUC of models on {dataset} train data for {TF}'), side=3, cex=1.2, line=2)
mtext('aggregation method/models', side=1, line=4)
text(xbp, 0.6, labels=round(tm_train, 3),cex=1,pos=3) 
```
...
tm_plt %>% 
    ...
    geom_text(aes(label=round(value, 2)), size = 4, vjust=-0.5, position = position_dodge(width=0.9))
```
...
```{r}
xplt <- tibble::column_to_rownames(tm, 'agg_model') %>% dplyr::arrange(desc(train)) |> as.matrix() |> t()

xbp <- barplot(xplt, beside=T, ylab='AUC', xaxt='n', ylim=c(0, 1))

# names.arg=colnames(xplt)

text(cex=0.9, x=xbp+0.05, y=0.0, labels=colnames(xplt), xpd=TRUE, srt=25, pos=2)

text(xbp, xplt + 0.01, labels=round(xplt, 2),cex=1,pos=3, srt=45)

mtext(glue('AUC of {TF} models on {dataset} data'), side=3, cex=1.2, line=2)
mtext('aggregation method/models', side=1, line=4)
```
