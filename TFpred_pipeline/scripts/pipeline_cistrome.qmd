...
useColors <- RColorBrewer::brewer.pal(n=length(useColors), name='Dark2')
useColors
```
...
temps <- purrr::map(.x=names(models_coefficients), function(each_aggm){
    plot_and_annotate_coefficients(models_coefficients[[each_aggm]], 'GATA3', each_aggm)
})
```
...
mtext(glue('AUC of models on {id} test data for {TF}'), side=3, cex=1.2, line=2)
