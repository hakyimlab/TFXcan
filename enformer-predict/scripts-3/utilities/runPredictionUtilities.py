...
def enformer_predict(sequence, region, sample, seq_type, model_path, model_func, output_dir, script_path, logfile_path):
    ...
    try:
        #enformer_model = model_func() # this load the model
...
def run_single_predictions(region, individual, vcf_func, fasta_func, script_path, output_dir, logfile, model_path, model_func, logfile_path): #
    ...
    check_result = check_query(sample = individual, query = region, output_dir=output_dir, logfile=logfile) #for region in region_batch
    #print(check_result)

    if check_result is not None:
        #print(f'{chk} is not None')
        b = create_individual_input_for_enformer(region=check_result, individual=individual, vcf_func=vcf_func, fasta_func=fasta_func, hap_type = 'hap1', resize_for_enformer=True, resize_length=None)

        if (b is not None) and (len(b['sequence']) == 393216): #(b['sequence'] is not None) and (len(b['sequence']) == 393216):
            reg_prediction = enformer_predict(b['sequence'], region=b['region'], sample=individual, seq_type=b['sequence_source'], model_path = model_path, model_func=enformer_model, output_dir=output_dir, script_path=script_path, logfile_path=logfile_path)
