...
### Plot the distribution of the sizes of the CWAS peaks
```{r}
...
cwas_loci_hg38 <- with(cwas_loci_hg38, GenomicRanges::GRanges(chr, IRanges(start, end), strand='+', score=0))
cwas_widths <- GenomicRanges::width(cwas_loci_hg38)
med <- median(cwas_widths)

#define histogram break points
hist_breaks <- seq(1, max(cwas_widths), by=10) #hist(cwas_widths)$breaks

#define colors to use in histogram based on break points
color_list <- rep('grey', length(hist_breaks))
color_list[hist_breaks == median(cwas_widths)] <- 'red'
cwas_widths %>% hist(main='Distribution of CWAS AR peaks', xlab='peak length/size', breaks = seq(1, max(.), by=10), col = color_list)
```

### What about the peaks for each individual AR chIP-seq data
```{r}
# read each bed file and turn into a granges object too
median_peak_size_per_individual <- lapply(cwas_bed_files, function(each_bed){
    ...
    median_length <- GenomicRanges::width(gfile) |> median()
    return(median_length)
}) |> unlist() %>% hist(main='Distribution of median AR peaks per individual', xlab='median peak length/size per individual', breaks = seq(1, max(.), length.out=100))
```
