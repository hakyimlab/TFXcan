...
```{r}
# predictions
tfpred_predictions <- readRDS(glue('{experiment_dir}/output/imputed_cwas_predictions/aggByMeanCenter_AR_Prostate_cwas.linear.rds'))
# individuals
...
```{r}
<<<<<<< HEAD
a <- cwas_scores_mat[1, ] |> unlist() |> unname()
b <- tfpred_mat[1,] |> unlist() |> unname()
reg_test <- lm(a ~ b) |> summary() |> coef()
reg_test[2, c(1,4)]
```
...
```{r}
sum(bh_corrected < (0.01/length(df_cor_test$p_value))) / length(df_cor_test$p_value)
```
...
<<<<<<< HEAD
```{r}
df_cor_test %>% dplyr::arrange('r') %>% ggplot(aes(r, -log10(p_value))) + geom_point() + theme_minimal() + labs(x='r, correlation coefficient', y='-log10(p)', title='r vs p-value of CWAS and TFPred correlation test for \nAR binding in CWAS individuals') + ylim(0, 1000)
```

### regression
```{r}
region_reg_test <- purrr::map(1:length(common_regions), function(i){
    a <- cwas_scores_mat[i, ] |> unlist() |> unname()
    b <- tfpred_mat[i,] |> unlist() |> unname()
    reg_test <- lm(a ~ b) |> summary() |> coef()
    out <- reg_test[2, c(1,4)]
    return(c(beta=out[1], pvalue=out[2]))
}, .progress=T)

df_reg_test <- do.call('rbind', region_reg_test) %>% as.data.frame() %>% data.table::setDT()
colnames(df_reg_test) <- c('beta', 'pvalue')
rownames(df_reg_test) <- common_regions
# filter out NAs 
df_reg_test <- df_reg_test %>% tibble::rownames_to_column('loci') %>% dplyr::filter(!(is.na(pvalue) | is.na(r)))
df_reg_test[1:5, ]
```

```{r}
#par(mfrow=c(1, 2), oma = c(0, 0.5, 2, 0), mar=c(4,4,2,2))
hist(df_reg_test[, 'beta'] |> unlist(), main='', xlab=expression(italic(beta) ~ ", effect"), cex.lab=1, col='brown')
mtext(expression("Histogram of" ~ beta ~ "(regression)"), side=3, line=2, adj=0.05, cex=1.2) 
mtext(glue("CWAS scores vs. TFPred scores for {nrow(df_reg_test)} regions"), side=3, line=0, cex=1.2, adj=0.05)
```
...
```{r}
hist(df_reg_test[, 'pvalue'] |> unlist(), main='', xlab=expression(italic(p)*"-value"), cex.lab=1, col='brown')
mtext(expression("Histogram of" ~ italic(p)*"-value" ~ "(spearman correlation)"), side=3, line=2, adj=0.05, cex=1.2) 
mtext(glue("CWAS scores vs. TFPred scores for {nrow(df_cor_test)} regions"), side=3, line=0, cex=1.2, adj=0.05)
```
...
```{r}
df_cor_test %>% dplyr::arrange('r') %>% ggplot(aes(r, -log10(p_value))) + geom_point() + theme_minimal() + labs(x='r, correlation coefficient', y='-log10(p)', title='r vs p-value of CWAS and TFPred correlation test for \nAR binding in CWAS individuals') + ylim(0, 70)
```
