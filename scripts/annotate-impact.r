...
# This pipeline works with Freedman's data
# Author: Temi
# Date: CircaAug 11 2022
# 
...
peak_files_dir <- glue('{work_dir}/processed-data/FOXA1/overlapped_29')
homer_dir <- '~/miniconda3/envs/r-env/share/homer'
project_dir <- '../'
output_dir <- glue('{work_dir}/processed-data/FOXA1/overlapped_29')

# chip file 
chip_data <- glue('{peak_files_dir}/input_chip.bed')
TF_CHIP_all <- read.table(chip_data, sep='\t')

# ensure that the output_dir is created
if(dir.exists(paths=output_dir)){
    print(paste(output_dir, 'exists.', sep=' '))
} else {
    dir.create(path=output_dir, recursive=T)
    print(paste(output_dir, 'created.', sep=' '))
}
...
chip_peak_headers <- c('chr', 'start', 'end')

colnames(TF_CHIP_all) <- chip_peak_headers
TF_CHIP_all[, 'strand'] <- '*'
TF_CHIP_all <- with(TF_CHIP_all, GRanges(chr, IRanges(start, end), strand)) |> as.data.frame()
#TF_CHIP_all <- reduce(TF_CHIP_GRANGES)
...
if(!file.exists(glue('{output_dir}/{TF}_{cell_line}.motif'))){
    ...
    cmd <- glue('cp {homer_dir}/data/knownTFs/motifs/{tolower(foxa1_motif)} {output_dir}/{TF}_{cell_line}.motif')
    system(cmd)
...
if(!file.exists(glue('{output_dir}/{TF}_{cell_line}_scanMotifsGenomewide_sort_15000.txt'))){
    cmd <- glue('perl {homer_dir}/bin/scanMotifGenomeWide.pl {output_dir}/{TF}_{cell_line}.motif {homer_dir}/data/genomes/hg19 > {output_dir}/{TF}_{cell_line}_scanMotifsGenomewide.txt')
    system(cmd)
...
ms_1 <- base::match(start(int_1) - 1, inst_bed$start)
