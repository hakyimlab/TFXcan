...
TF_INFO <- hf_info[hf_info$Factor == TF & hf_info$Cell_line == cell_line, c('DCid', 'Cell_line', 'Cell_type', 'Tissue_type')]

chip_peak_headers <- c('chrom', 'start', 'end', 'name', 'score', 'strand', 'signalValue', 'pValue', 'qValue', 'peak')
...
if(!TF %in% hf_info$Factor){
    ...
    TF_INFO <- hf_info[hf_info$Factor == TF & hf_info$Cell_line == cell_line, c('DCid', 'Cell_line', 'Cell_type', 'Tissue_type')] # across different cell lines and tissues
    ...
    TF_CHIP <- lapply(seq_along(TF_CHIP), function(i){
        ...
        cbind(temp[, 1:6], each_chip_extra_info)
    })
...
# overlaps <- findOverlaps(TF_CHIP_GRANGES)

# TF_CHIP_GRANGES <- lapply(TF_CHIP, function(tfc){
#     colnames(tfc)[1:6] <- c('chr','start','end','id','score','strand')
#     tfc[, 'strand'] <- '*'
#     with(tfc, GRanges(chr, IRanges(start, end), strand, score, id = id))
# }) |> GRangesList()

TF_CHIP_all <- do.call(rbind, TF_CHIP)
colnames(TF_CHIP_all)[1:6] <- c('chr','start','end','id','score','strand')
TF_CHIP_all[, 'strand'] <- '*'
TF_CHIP_GRANGES <- with(TF_CHIP_all, GRanges(chr, IRanges(start, end), strand, score, id = id))
TF_CHIP_all <- reduce(TF_CHIP_GRANGES) |> as.data.frame()

#TF_CHIP_GRANGES <- as(TF_CHIP_GRANGES, 'GRangesList')
...
TF_BED <- unique(cbind(TF_CHIP_all[, c(1:3)], 0, motif, "+"))
...
write.table(tf_bed, glue("{output_dir}/{TF}_{cell_line}_train_df.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)

if(!file.exists(glue('{output_dir}/{TF}_{cell_line}.motif'))){
    # copy the motif file from homer
    cmd <- glue('cp {homer_dir}/data/knownTFs/motifs/{tolower(TF)}.motif {output_dir}/{TF}_{cell_line}.motif')
    system(cmd)
    print(glue('{output_dir}/{TF}_{cell_line}.motif copied!'))
} else {
    print(glue('{output_dir}/{TF}_{cell_line}.motif exists'))
}


# 
pwm <- read.table(glue('{output_dir}/{TF}_{cell_line}.motif'), sep = "\t", header = F, stringsAsFactors = FALSE, fill=T)
pwm[1, 4] <- 'PH'
pwm[1, 2] <- glue('{TF}_{cell_line}') # NOTE THIS PART CAREFULLY
write.table(pwm[, 1:4], glue('{output_dir}/{TF}_{cell_line}.motif'), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
...
write.table(MotifThreshinfo, glue('{output_dir}/{TF}_{cell_line}_motif_threshold_info.txt'), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)

# create the genome wide motif files
if(!file.exists(glue('{output_dir}/{TF}_{cell_line}_scanMotifsGenomewide_sort_15000.txt'))){

    cmd <- glue('perl ~/miniconda3/share/homer/bin/scanMotifGenomeWide.pl {output_dir}/{TF}_{cell_line}.motif ~/miniconda3/share/homer/data/genomes/hg19 > {output_dir}/{TF}_{cell_line}_scanMotifsGenomewide.txt')
    system(cmd)
    cmd <- glue("sort -t $'\t' -k6,6rn {output_dir}/{TF}_{cell_line}_scanMotifsGenomewide.txt > {output_dir}/{TF}_{cell_line}_scanMotifsGenomewide_sort.txt")
    system(cmd)
    cmd <- glue('head -n 15000 {output_dir}/{TF}_{cell_line}_scanMotifsGenomewide_sort.txt > {output_dir}/{TF}_{cell_line}_scanMotifsGenomewide_sort_15000.txt')
    system(cmd)
 
} else {
    print(glue('{output_dir}/{TF}_{cell_line}_scanMotifsGenomewide_sort_15000.txt exists.'))
}

#homer find instances of motifs

if(!file.exists(glue('{output_dir}/{TF}_{cell_line}_findinstances.txt'))){
    cmd <- glue('perl {homer_dir}/bin/findMotifsGenome.pl {output_dir}/{TF}_{cell_line}_train_df.txt {homer_dir}/data/genomes/hg19 {project_dir}/processed-data/homer_output/ -size given -find {output_dir}/{TF}_{cell_line}.motif > {output_dir}/{TF}_{cell_line}_findinstances.txt')
    system(cmd)
} else {
    print(glue('{output_dir}/{TF}_{cell_line}_findinstances.txt exists.'))
}

# STEP 2 : prepare the positive and negative sets
motif_instances <- read.table(glue('{output_dir}/{TF}_{cell_line}_findinstances.txt'), sep = "\t", header = T, stringsAsFactors = FALSE)
chip_regions <- read.table(glue('{output_dir}/{TF}_{cell_line}_train_df.txt'), sep='\t', header=F, stringsAsFactors=F) #as.data.frame(tf_bed) # or read.table(glue('{output_dir}/{TF}_train_df.txt'), sep='\t', header=T, stringsAsFactors=F)
motif_library <- read.table(glue('{output_dir}/{TF}_{cell_line}_motif_threshold_info.txt'), sep = "\t", header = F, stringsAsFactors = FALSE)
...
write.table(chip_regions_newpeaks, glue("{output_dir}/{TF}_{cell_line}_train_df_newpeaks.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
...
write.table(train_test_peaks, glue("{output_dir}/{TF}_{cell_line}_train_test_positive_bed.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)

# STEP 3 : prepare the negative sets

train_df_newpeaks <- read.table(glue("{output_dir}/{TF}_{cell_line}_train_df_newpeaks.txt"), sep = "\t", header = F, stringsAsFactors = FALSE)

MotifThreshold <- read.table(glue("{output_dir}/{TF}_{cell_line}_motif_threshold_info.txt"), sep = "\t", header = F, stringsAsFactors = FALSE)

motifs <- MotifThreshold[1,1]
instscan <- read.table(glue("{output_dir}/{TF}_{cell_line}_scanMotifsGenomewide_sort_15000.txt"), sep = "\t", header = F, stringsAsFactors = FALSE)
instscan <- instscan[order(instscan[,6], decreasing = T),]   
train_df <- read.table(glue("{output_dir}/{TF}_{cell_line}_train_df.txt"), sep = "\t", header = F, stringsAsFactors = FALSE) #old version: ChIPbed1.txt
...
write.table(noChIP, glue("{output_dir}/{TF}_{cell_line}_noChIP.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
...
write.table(noChIP, glue("{output_dir}/{TF}_{cell_line}_train_test_negative_bed.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
