...
setwd('/projects/TFXcan/imlab/users/temi/projects/TFXcan/scripts/')
...
bedfiles_dir <- '/lus/grand/projects/TFXcan/imlab/data/baca_cwas/bed'
...
homer_dir <- '/lus/grand/projects/TFXcan/imlab/users/temi/software/homer/bin'

#output_dir <- glue('../processed-data/{TF}/{cell_line}/freedman')

peak_files_full_path <- list.files(bedfiles_dir, full.names=T)
#bed_files <- peak_files_full_path[startsWith(list.files(glue('{peak_files_dir}')), TF)] # only the TF files
bedfiles <- lapply(peak_files_full_path, data.table::fread) # read in the files into like a list

#FOXA1_bed_files <- peak_files_full_path[startsWith(list.files(glue('{peak_files_dir}')), TF)] # only the TF files
file_names <- sapply(strsplit(x = bedfiles, split='/', fixed=T), function(ea){
    ea[12]
})
...
glue('{bedtools_cmd} multiinter -header -i (find {bedfiles_dir}) > intersected_files.bed')
