...
Here I select those with a score >= 11
```{r}
...
```{r}
set.seed(2022)
chip_dt <- chip_dt[sample(1:nrow(chip_dt), 10000, replace=F), ]
nrow(chip_dt)
```
```{r}
...
```{r}
find_motifs_script <- glue("{project_dir}/scripts/find_motifs_genome.sh")
find_motifs_script
```
...
```{r, eval=F}
cmd <- glue("qsub -v peaks={output_dir}/{id}_{TF}_{cell_line}_train_df.txt,genome={homer_dir}/data/genomes/hg19,homer_output={output_dir}/homer_output/,size=given,motif_file={common_dir}/{TF}_{cell_line}_threshold.motif,output={output_dir}/{id}_{TF}_{cell_line}_findinstances.txt {find_motifs_script}")
system(cmd)
```
...
### Step 7: Check that there are not overlaps between positives and negatives

```{r}
pos_granges <- with(train_df_newpeaks, GenomicRanges::GRanges(V1, IRanges(V2,V3), strand=V6, score=V5, id = V4))
pos_granges
```
