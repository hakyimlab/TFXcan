...
def polaris_htParslConfig(params):
    ...
    logpath = os.path.join(workingdir, 'scheduler_log')
    ...
    sch_options = ['#PBS -l filesystems=home:grand:eagle',
                    f'#PBS -N {job_name}',
                    #f'#PBS -o {workingdir}/scheduler_log/{job_name}.out',
                    #f'#PBS -e {workingdir}/scheduler_log/{job_name}.err',
                    f'#PBS -k doe'
    ]
    ...
    cpus_per_node = 64 #* params['num_of_full_nodes']
...
#                 worker_debug=True,
#                 cores_per_worker=56, # how many cores per worker #nodes_per_block, 2 is usually enough or 1.
#                 working_dir=workingdir,
#                 address=address_by_hostname(),
...
#                 available_accelerators=4,
#                 worker_debug=True,
#                 cores_per_worker=56, # how many cores per worker 
#                 working_dir=workingdir,
#                 address=address_by_hostname(),
#                 provider=LocalProvider(
#                     channel=LocalChannel(),
