...
all_test_linear_eval %>%
    ...
    theme_bw() +
    labs(title='Density plot of linear models', y='density', x=expression(X*beta)) +
...
ggplot(res_for_plot, aes(x=data_type, y=factor(agg_method), fill=cor)) + 
    ...
    theme_classic() +
    theme(text = element_text(size = 15))  + 
    labs(title=glue("Correlation of {TF} Cistrome models"),
        x='data type', y='aggregation method/model') +
    scale_fill_gradient(low = "white", high = "red") +
...
response_pred <- predict(logistic_models_list$aggByMean, newx, s="lambda.1se", type='response')
...
plot_and_annotate_coefficients <- function(coeff_dt, TF, agg_method='aggByCenter', annotate_dt=list()){
    ...
    pdt %>%
        ...
        labs(title=glue('Plot of coefficients of {TF} {agg_method} predictors'), x = 'non-zero tracks/predictors', y='coefficients') +
