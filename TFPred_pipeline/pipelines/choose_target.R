...
# curate enformer
ue <- enformer_db$target |> sort()
a <- gsub('3xFLAG-', '', ue)
ue <- gsub('eGFP-', '', a) |> sort() |> unique()
...
# ===== 
TF_db <- data.table::fread(glue('{project_dir}/metadata/transcription_factor_availability_2.txt'), fill=T)
