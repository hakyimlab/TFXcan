...
ones <- a[a$class == 1, ]$aggByPreCenter
zeros <- a[a$class == 0, ]$aggByPreCenter
...
purrr::map(.x=agg_methods, function(each_agg){
    ...
    bpt <- all_pscores %>%
        ...
        ggplot(.) + aes(x=class, y=.data[[each_agg]], fill=class) + 
    ...
    dt <- all_pscores %>%
        ...
        dplyr::summarise(group_mean = mean(.data[[each_agg]]))

    
    dpt <- all_pscores %>%
        dplyr::mutate(class= as.factor(class)) %>%
        ggplot(., aes(x=.data[[each_agg]], fill=class)) + 
        ...
        geom_vline(data = dt, aes(xintercept = group_mean), color='black',linetype="dashed") +
        theme(axis.title=element_text(size=12),
            plot.title=element_text(size=15),
...
dt <- all_pscores %>%
    dplyr::group_by(class) %>%
    dplyr::summarise(group_mean = mean(aggByPostCenter))
```
...
dpt <- all_pscores %>%
    dplyr::mutate(class= as.factor(class)) %>%
    ggplot(., aes(x=aggByCenter, fill=class)) + 
    geom_density(alpha = 0.8) + theme_tufte() + geom_rangeframe() + labs(title='Predicted scores across all regions and individuals', y='density') + 
    geom_vline(data = dt, aes(xintercept = group_mean), color='black',linetype="dashed") +
    theme(axis.title=element_text(size=12),
        plot.title=element_text(size=15),
...
```{r}
qqplot_extended(pvalues_regions$V2, frame.plot=F)
mtext('qqplot of p-values (t-test)', at=0.5, line=1, cex=1.5)
```
...
mtext('Histogram of p-values', cex=1.5, at=0.2)
```
...
```{r}
m0 <- pscores_dt %>% 
    dplyr::select(region, class, aggByPreCenter, individual) %>%
    dplyr::filter(class == 0) %>%
    dplyr::group_by(region) %>% 
    dplyr::summarise(mean_0 = mean(aggByPreCenter)) %>%
...
m1 <- pscores_dt %>% 
    ...
    dplyr::group_by(region) %>% 
    dplyr::summarise(mean_1 = mean(aggByPreCenter)) %>%
...
mtext('Histogram of mean differences', cex=1.5, adj=0)
```
...
pscores_top_regions %>%
    ...
    geom_density() + 
    facet_wrap(~region, scales='free') + theme_bw() +
    labs(title='Density plot of top 9 regions (ranked by p-values)', y='density') +
    theme(plot.title=element_text(size=20), 
        axis.title=element_text(size=18))
```
...
pt <- prb_counts %>%
    ...
    dplyr::summarise(counts = sum(pvalue <= 0.001)) %>% t()
...
mtext('Number of regions with pvalues <= 0.001 \nper binding count group', cex=1.5, adj=0.25, line=0)
