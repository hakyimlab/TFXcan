...
vcfs_dir <- glue('{imlab_dir}/data/freedman_vcfs/vcfs_2023-01-25') # /lus/grand/projects/covid-ct/imlab/data/freedman_vcfs/vcfs_2023-01-25
...
```{r}
vcf_files <- list(folder=vcfs_dir, files=vcf_files)
vcf_files
```
...
enformer_parameters_json[['write_log']] <- list('logdir' = glue('job_logs'), 'logtypes' = list('memory'=FALSE, 'error'=TRUE, 'time'=FALSE, 'cache'=FALSE))
...
predict_on <- glue('{project_dir}/metadata/freedman_individuals.txt')
...
individuals <- data.table::fread(glue('/lus/grand/projects/covid-ct/imlab/users/temi/projects/TFXcan/TFPred_pipeline/metadata/{dataset}_individuals.txt'), header=F)
...
individual_aucs %>% tibble::rownames_to_column('agg_method') %>% tidyr::pivot_longer(!agg_method) %>% ggplot(.) + aes(x=agg_method, y=value) + 
geom_boxplot() + geom_jitter() + theme_bw() + 
    ...
    labs(title='AUC of TFPred predictions across individuals per aggregation method', x='aggregation method',y='AUC')
#barplot(t(individual_aucs), beside=T)
```

only aggbymean
```{r}
individual_aucs %>% tibble::rownames_to_column('agg_method') %>% tidyr::pivot_longer(!agg_method) %>% dplyr::filter(agg_method == 'aggByMean') %>% ggplot(.) + aes(x=agg_method, y=value) + 
geom_boxplot() + geom_jitter() + theme_bw() + 
    theme(panel.border=element_blank(),
        axis.text = element_text(size = 12, color='black'),
        ...
        axis.title = element_text(size=15, color='black'),
        plot.title= element_text(size=18)) +
    labs(title='AUC of TFPred predictions across individuals', x='',y='AUC')
```
...
```{r}
# pick and individuals and plot against

individual_aucs %>% tibble::rownames_to_column('agg_method') %>% dplyr::select(agg_method, LuCaP_173) %>% dplyr::left_join((random_AUC %>% as.data.frame() %>% tibble::rownames_to_column('agg_method')), by='agg_method') %>% tidyr::pivot_longer(!agg_method) %>% ggplot(.) + 
    aes(x=name, y=value) + geom_boxplot(width=0.5) + 
    geom_jitter() + labs(x='individual or random data', y='AUC', title='AUC of TFPred\'s performance on \none individual vs random set/genome') +
    theme_bw() + 
    theme(panel.border=element_blank(),
        axis.text = element_text(size = 12, color='black'),
...
temps <- purrr::map(.x=names(linear_models_coefficients), function(each_aggm){
    res_for_plot %>% 
        dplyr::filter(agg_method == each_aggm) %>%
        ggplot(.) + aes(x=individual, y=AUC) +
...
est <- round(t_test_result$estimate, 2) |> unname()
conf_int <- round(t_test_result$conf.int, 3)
report_label <- glue('t-test\nmean_0 = {est[1]}, mean_1 = {est[2]}\np < {ifelse(pv > 0, pv, "2.2e-16")}\n95% conf. int.: {conf_int[1]} to {conf_int[2]}')
```

```{r}
at.x <- seq(1,by=.5,length.out=2)

boxplot(dt$predicted ~ dt$observed, frame.plot=F, col='orange', ylab=expression(beta * X ~ "/ Predicted TFPred Scores"), xlab='observed class', pch=20, boxwex=0.3, at = at.x)
#points(est,col="red",pch=18, cex=2)
mtext(glue('Distribution of {use_method} {TF}-linear scores \nacross the genome for all individuals'), adj=0, cex=1.5, line=0)
mtext(report_label, adj = 0, line=-4, cex=1.0)
#dev.off()
```
...
dt %>% mutate(observed=as.factor(observed)) %>%
    ...
    geom_signif(annotation = ttest_pvalues, tip_length = 0.02, y_position = ytip, xmin = xmins, xmax = xmaxs) + theme_minimal() +
    labs(title=glue('{use_method} {TF}-linear scores by individual across the genome'), y='TFPred score') + theme(axis.text.x=element_text(angle=45, color='black'), axis.line.x = element_blank())
...
```{r}
pscores_list[[ind_names[1]]]$region[1:5]

pscores_list$aggByPreCenter[[2]][1:5, ]
```
...
mtext(glue('histogram of t-test pvalues \nacross 17 individuals for each loci'), adj=0, cex=1.5,line=0)
```

```{r}
hist(tvec, xlab='t-statistic', main='')
mtext(glue('distribution of t-statistic \nacross 17 individuals for each loci'), adj=0.1, cex=1.5,line=0)
```
...
```{r}
qqunif(pvec, frame.plot=F, pch=20) 
mtext(glue('qqplot of t-test pvalues \nacross 17 individuals for each loci'), adj=0, cex=1.5)
```
